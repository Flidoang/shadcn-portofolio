    ...
    <div class="mx-auto max-w-7xl relative pb-20">
      <div v-for="(item, index) in items" :key="index" :ref="(el) => { if(el) itemRefs[index] = el as HTMLElement }" class="flex justify-start pt-10 md:pt-40 md:gap-10">
        <div class="sticky top-40 z-40 flex max-w-xs flex-col items-center md:w-full md:flex-row lg:max-w-sm">
          <div class="absolute left-3 flex h-10 w-10 items-center justify-center rounded-full bg-neutral-100 dark:bg-neutral-900 md:left-3 shadow-sm border border-neutral-200 dark:border-neutral-800">
            <div 
              class="h-4 w-4 rounded-full border border-neutral-300 bg-neutral-200 dark:border-neutral-700 dark:bg-neutral-800 transition-colors duration-500" 
              :class="activeIndex === index ? 'bg-purple-500 border-purple-500 dark:bg-purple-500' : ''"
            />
          </div>
          <h3 
            class="hidden text-xl font-bold md:block md:pl-20 md:text-5xl transition-colors duration-500"
            :class="activeIndex === index ? 'text-purple-600 dark:text-purple-400' : 'text-neutral-500 dark:text-neutral-500'"
          >
            {{ item.label }}
          </h3>
        </div>
        ...
        <div class="relative w-full pr-4 pl-20 md:pl-4">
          <h3 class="mb-4 block text-2xl font-bold text-neutral-500 dark:text-neutral-500 md:hidden">
            {{ item.label }}
          </h3>
      ...
      <div class="absolute left-8 top-0 w-[2px] h-full bg-neutral-200 dark:bg-neutral-800"></div>

      <Motion
        as="div"
        :style="{ height: heightTransform, opacity: opacityTransform }"
        class="absolute left-8 top-0 w-[2px] bg-linear-to-t from-purple-500 via-blue-500 to-transparent from-0% via-10% rounded-full"
      />
    </div>
  </div>
</template>
