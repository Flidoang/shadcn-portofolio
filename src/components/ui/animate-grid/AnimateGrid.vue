...
<template>
  ...
  'relative grid w-full max-w-4xl items-center justify-center gap-6 p-4',
  props.cards.length < 4 ? `grid-cols-${props.cards.length}` : 'grid-cols-2 sm:grid-cols-3 md:grid-cols-4',
)">
      <div v-for="(item, index) in props.cards" :key="index" ref="card" class="card flex items-center justify-center rounded-xl border border-transparent p-6 transition-all duration-200 aspect-square min-w-[90px] sm:min-w-[110px] md:min-w-[130px]" :style="{
  zIndex: index + 1,
}">
